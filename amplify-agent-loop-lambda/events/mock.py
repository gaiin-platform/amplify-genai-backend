...
"""
    ...
    ses_event = {
        "Records": [
            {
                "messageId": str(uuid.uuid4()),
                "receiptHandle": "AQEB" + "".join([str(uuid.uuid4()).replace("-", "") for _ in range(4)]),
                "body": body_content,
                "attributes": {
                    "ApproximateReceiveCount": "1",
                    "SentTimestamp": str(int(datetime.now(timezone.utc).timestamp() * 1000)),
                    "SenderId": "AIDAIT2UOQQY3AUEKVGXU",
                    "ApproximateFirstReceiveTimestamp": str(int(datetime.now(timezone.utc).timestamp() * 1000))
                },
                "messageAttributes": {},
    ...
    if cc:
        ...
        ses_notification["mail"]["headers"].append({"name": "CC", "value": ", ".join(cc)})
    if bcc:
        ses_notification["mail"]["commonHeaders"]["bcc"] = bcc
        ses_notification["mail"]["headers"].append({"name": "BCC", "value": ", ".join(bcc)})
