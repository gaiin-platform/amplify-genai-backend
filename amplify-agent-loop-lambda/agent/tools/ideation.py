...
def critique_and_improve_content(action_context:ActionContext, content:str):
    ...
    updated_content = generate_response(Prompt(messages=[
        {"role": "user", "content": f"Apply the following critique to the content to improve it:\n\n{critique}\n------------\nContent:\n{content}"}
    ]))
...
def ideate(action_context:ActionContext, generator_instructions:str, iterations=3, improve_content=False):
    ...
    for i in range(iterations):
        ...
        response = generate_response(Prompt(messages=[{"role": "user", "content": generator_instructions}]))
    ...
    for i in range(iterations):
        ...
        response = generate_response(Prompt(messages=[
            {"role": "user", "content": f"Create a highly detailed variation on this prompt that will generate foundational, but unique variations of the content. "
    ...
    for i in range(iterations):
        ...
        response = generate_response(Prompt(messages=[{"role": "user", "content": variation_prompts[i]}]))
    ...
    best_of_breed = generate_response(Prompt(messages=[
        {"role": "user", "content": f"Combine the best parts of this content and output it as a single, cohesive response. \n\n{combined_content}"}
    ]))
