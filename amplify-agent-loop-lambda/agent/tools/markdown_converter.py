...
def convert_to_markdown(file_path: str, output_path: Optional[str] = None, ocr: bool = False, 
                       describe_images: bool = False, extract_images: bool = False, 
                       language: str = "en", timeout: int = 120):
    """
        ...
        if extract_images:
            ...
            if os.path.exists(images_dir) and os.path.isdir(images_dir):
                for image_file in os.listdir(images_dir):
                    if os.path.isfile(os.path.join(images_dir, image_file)) and _is_image_file(image_file):
...
@register_tool(tags=["file_handling", "markdown"])
def batch_convert_to_markdown(file_paths: List[str], output_dir: str, ocr: bool = False,
                             describe_images: bool = False, extract_images: bool = False,
                             language: str = "en", timeout: int = 300):
    """
    ...
    per_file_timeout = max(timeout // max(len(file_paths), 1), 30)  # Minimum 30 seconds per file
...
@register_tool(tags=["file_handling", "markdown"])
def convert_url_to_markdown(url: str, output_path: Optional[str] = None, 
                          include_images: bool = True, timeout: int = 180):
    """
