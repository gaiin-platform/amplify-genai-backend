...

{
    ...
    "db_type": "snowflake|postgres|mysql|mssql|duckdb|bigquery|sqlite|oracle",
...
def load_config_from_dynamodb(current_user: str = None):
    ...
    try:
        ...
        for item in items:
            ...
            # Assuming each item has a 'connection_id' as the primary key
            # and contains database configuration details
            connection_id = item.get("connection_id", "default")

            # Extract database configuration from the item
            db_config = {
                "db_type": item.get("db_type", "snowflake"),
...
def get_db_config(connection_id: str = "default", current_user: str = None) -> Dict:
    ...
    return config["db_config"].get(
        connection_id, config["db_config"].get("default", {})
    )
    ...
    def _get_relevant_schema(self, question: str) -> dict:
        ...
        for table_name, columns in self.db_schema.items():
            ...
            if table_lower in question_lower:
                ...
                for col in columns:
                    col_name = col.split(" ")[0].lower()  # Get column name without type
