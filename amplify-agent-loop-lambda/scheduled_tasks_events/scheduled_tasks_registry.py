...
def create_scheduled_task(current_user, task_name, description, task_instructions, task_type, 
                         object_info, cron_expression, date_range=None, active=True, 
                         tags=None, notify_on_completion=False, notify_on_failure=False, 
                         notify_email_addresses=None, access_token=None, account=None, time_zone=None):
    """
...
def get_scheduled_task(current_user, task_id):
    ...
    try:
        ...
        task = {key: deserializer.deserialize(value) for key, value in response['Item'].items()}
...
def update_scheduled_task(current_user, task_id, task_name=None, description=None, 
    ...
    try:
        ...
        def add_to_update(field_name, param_value, dynamo_field=None):
            if param_value is not None:
                ...
                expression_attribute_values[placeholder] = serializer.serialize(param_value)
...
def delete_scheduled_task(current_user, task_id):
    """
...
def get_task_execution_details(current_user, task_id, execution_id):
    ...
    try:
        ...
        logs = task.get('logs', [])
        execution_record = next((log for log in logs if log.get('executionId') == execution_id), None)
        ...
        if 'detailsKey' in execution_record:
            try:
                s3_response = s3.get_object(
                    Bucket=logs_bucket,
                    Key=execution_record['detailsKey']
                )
...
def execute_specific_task(current_user, task_id):
    ...
    try:
        ...
        # If task doesn't exist or doesn't belong to the user
        if 'Item' not in response:
            return {"success": False, "message": "Task not found or you don't have permission to execute it"}
