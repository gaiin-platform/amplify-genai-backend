...
from common.actionSets import load_action_set
...
class TasksMessageHandler(MessageHandler):
    ...
    def process(self, message: Dict[str, Any], context: Any) -> Dict[str, Any]:
        try:
            ...
            print(f"Processing scheduled task: { task_data.get('taskName')} ({task_id}) for user {user_id}") 
            # Clear the lastCheckedAt field so the task can be retried
            if (source == "scheduled-task"): self._reset_task_check_status(user_id, task_id, last_run_at_utc)
            ...
            event_payload = {
                ...
                "metadata": {
                    ...
                    "files": [],
                    **self._resolved_object_info(user_id, task_data.get('objectInfo', {}), task_type, api_key)
                }
            }
            
            # Record that the task has started execution
            add_task_execution_record(user_id, task_id, 'running', {
                'sessionId': session_id,
    ...
    def onFailure(self, event: Dict[str, Any], error: Exception) -> None:
        ...
        if not user_id or not task_id:
            ...
            add_task_execution_record(user_id, task_id, 'failure', {
                    'error': error,
    ...
    def onSuccess(self, agent_input_event: Dict[str, Any], agent_result: Dict[str, Any]) -> None: 
        ...
        if not user_id or not task_id:
            print(f"Error in onSuccess: Missing task data in event: {agent_input_event}")
            return
    ...
    def _resolved_object_info(self, user, object_info, object_type, access_token):
        ...
        if object_type == "assistant":
            ...
            if not assistant_response["success"]:
                raise Exception(f"The scheduled taskexists, but its associated assistant could not be found.")
            return {"assistant": assistant_response["data"]}

        elif object_type == "actionSet":
            print(f"Processing actionSet with object_id: {object_id}")
            action_set = load_action_set(access_token, object_id)
            ...
            for action in actions:
                ...
                bindings = {k: v for k, v in defined_tool_parameters.items() if v["value"] != ""}
...
def task_completed(user_id, task_id, task_data, result):
    ...
    try:
        ...
        add_task_execution_record(user_id, task_id, 'success', {
            'result': result,
        ...
        if task_data.get('notifyOnCompletion', False) and task_data.get('notifyEmailAddresses', []): 
   ...
   {extract_task_detail_message(task_data)}
    ...
    """
            ...
            email_task_details(api_key, email_subject, email_body, task_data.get('notifyEmailAddresses'))
...
def task_failed(user_id, task_id, error, task_data):
    ...
    try:
        ...
        if task_data.get('notifyOnFailure', False) and task_data.get('notifyEmailAddresses', []):   
            # Prepare rich notification content
            task_name = task_data.get('taskName', 'Unknown Task')
...
def get_timestamp(isoformat=True):
    ...
    if (isoformat): date = date.isoformat()
    return date
...
def extract_task_detail_message(task_data):
    ...
    object_name = object_info.get('objectName', 'Unknown Object') if isinstance(object_info, dict) else 'Unknown Object'
...
def add_task_execution_record(current_user, task_id, status, details=None):
    ...
    if not table_name or not logs_bucket:
        raise ValueError("Environment variables 'SCHEDULED_TASKS_TABLE' and 'SCHEDULED_TASKS_LOGS_BUCKET' must be set.")
    ...
    try:
        ...
        for page in page_iterator:
            for item in page.get('Items', []):
                task = {key: deserializer.deserialize(value) for key, value in item.items()}
                user = task.get('user')
                ...
                try:
                    ...
                    print(f"Invalid timezone '{user_timezone_str}' for task {task_id}. Using America/Chicago.")
                ...
                if not cron_expression:
                    print(f"Task {task_id} for user {user} is active but has no cronExpression. Skipping.")
                    continue
                ...
                print(f"Current time in user's timezone: {now_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')}")
                ...
                if 'startDate' in date_range and date_range['startDate']:
                    try:
                        ...
                        if isinstance(start_date_str, str):
                            # Ensure start_date is UTC aware for comparison with now_utc
                            ...
                            if now_utc < start_date:
                                ...
                                print(f"Task not started yet. Start date: {start_date.isoformat()}")
                    except (ValueError, TypeError) as e:
                        print(f"Error parsing start date for task {task_id} (user {user}): {e}. Skipping.")
                        skip_task_due_to_date_range = True
                
                if not skip_task_due_to_date_range and 'endDate' in date_range and date_range['endDate']:
                    try:
                        ...
                        if isinstance(end_date_str, str):
                            ...
                            if now_utc > end_date:
                                skip_task_due_to_date_range = True
                                print(f"Task has ended. End date: {end_date.isoformat()}")
                    except (ValueError, TypeError) as e:
                        print(f"Error parsing end date for task {task_id} (user {user}): {e}. Skipping.")
                        skip_task_due_to_date_range = True
                ...
                if last_checked_at_str:
                    try:
                        ...
                        print(f"Error parsing lastCheckedAt '{last_checked_at_str}'. Treating as None.")
                        last_checked_at = None
                ...
                for date_type in attr:
                    ...
                    if date_str and date_str != "":
                        try:
                            ...
                            print(f"Using {date_type} as base (UTC): {base_dt_utc.isoformat()}")
                            break
                        except ValueError:
                ...
                if not base_dt_utc:
                    print(f"Task {task_id} has no valid lastRunAt or createdAt. Skipping.")
                    continue

                # Convert base time to user's timezone for cron calculation
                base_dt_user_tz = base_dt_utc.astimezone(user_tz)
                print(f"Base time in user timezone: {base_dt_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')}")
                ...
                try:
                    ...
                    while True:
                        ...
                        if next_instance_utc > now_utc:
                            ...
                            print(f"Next scheduled run: {next_instance_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')} ({next_instance_utc.strftime('%Y-%m-%d %H:%M:%S %Z')})")
                            break
                            
                        # Only include instances that haven't been checked yet
                        if last_checked_at is None or next_instance_utc > last_checked_at:
                            due_instances_utc.append(next_instance_utc)
                            print(f"  Found due instance: {next_instance_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')} ({next_instance_utc.strftime('%Y-%m-%d %H:%M:%S %Z')})")
                            
                        # Safety limit to prevent infinite loops
                        if len(due_instances_utc) > 1000:
                            print(f"WARNING: Task {task_id} has over 1000 due instances. Limiting to most recent 100.")
                    ...
                    earliest_due_instance_user_tz = earliest_due_instance_utc.astimezone(user_tz)
                    print(f"Will process earliest due instance: {earliest_due_instance_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')} ({earliest_due_instance_utc.isoformat()})")
                    if len(due_instances_utc) > 1:
                        print(f"  Note: {len(due_instances_utc) - 1} additional instance(s) pending")
                ...
                print(f"Attempting to claim task {task_id} for instance {earliest_due_instance_utc.isoformat()}")
                try:
                    ...
                    dynamodb.update_item(
                        ...
                        ExpressionAttributeValues={
                            ...
                            ":run_id_val": {"S": run_id},
                            ":earliest_instance_iso": {"S": earliest_due_instance_utc.isoformat()}
                        },
                        ReturnValues="NONE" 
                    )
                    
                    print(f"✓ Successfully claimed task {task_id} (user {user}) for instance {earliest_due_instance_user_tz.strftime('%Y-%m-%d %H:%M:%S %Z')}")
                    tasks.append(task) 
                    
                except dynamodb.exceptions.ConditionalCheckFailedException:
                    print(f"✗ Task {task_id} (user {user}) already claimed by another process")
                    continue
                except Exception as e:
                    print(f"✗ Error updating lastCheckedAt for task {task_id} (user {user}): {e}")
                    continue
        ...
        import traceback
        print(f"General error in find_tasks_to_execute (run {run_id}): {e}\n{traceback.format_exc()}")
...
def send_tasks_to_queue(tasks: List[Dict[str, Any]], task_source="scheduled-task"):
    ...
    for task in tasks:
        ...
        try:
            ...
            response = sqs.send_message(
                QueueUrl=queue_url,
                MessageBody=safe_json_dumps(message)
            )
    ...
    for failedTask in failed:
        ...
        add_task_execution_record(failedTask['userId'], id, 'failure', {
