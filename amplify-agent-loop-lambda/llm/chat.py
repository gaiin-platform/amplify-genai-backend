...
def chat_simple(access_token, model, system_instructions, prompt_instructions):
    ...
    if not access_token and not os.getenv("AMPLIFY_API_KEY"):
        raise ValueError("You must provide an access token or API key to @prompt. Please set the "
                         "environment variable AMPLIFY_API_KEY or pass the access_token.")
