...
def get_embedding_models():
    ...
    if (admin_table is None or model_rate_table is None):
        return {"success": False, "message": "AMPLIFY_ADMIN_DYNAMODB_TABLE or MODEL_RATE_TABLE not set"}
    ...
    try:
        ...
        return {"success": False, "message": "Error retrieving default models: {str(e)}"} 
    ...
    try:
        def get_provider(model_id, default_key):
            ...
            if 'Item' in response:
                item = response['Item']
                defaults[default_key] = {'model_id': item['ModelID'], 'provider': item['Provider']}
