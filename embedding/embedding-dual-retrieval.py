...
def classify_src_ids_by_access(raw_src_ids, current_user):
    ...
    try:
        ...
        logger.error(f"An error occurred while classifying src_ids by access: {e}")
    ...
    logger.debug(f"Accessible src_ids: {accessible_src_ids}, Access denied src_ids: {access_denied_src_ids}")
...
async def _async_process_input_with_dual_retrieval(event, context, current_user, name, data):
    ...
    if response_embeddings["success"]:
        ...
        logger.debug(f"Here are the token count {token_count}")
    else:
    ...
    logger.debug(f"Here are the related docs {related_docs}")
...
async def check_embedding_completion(src_ids, account_data, requeue_failures=None, max_requeue_failures=3):
    ...
    logger.info("Asynchronous embedding check complete")
