...
def replace_key(access_token, api_owner_id):
    print("Initiate replace key call")

    amplify_group_endpoint = os.environ['API_BASE_URL'] + '/apiKeys/replace_key'
    ...
    try:
        ...
        if response.status_code != 200  or not "success" in response_content:
            print("Error calling replace api key: ", response_content)
            return ''
        elif response.status_code == 200:
