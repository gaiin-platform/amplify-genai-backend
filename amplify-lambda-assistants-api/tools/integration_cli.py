...
def list_integrations(show_details):
    ...
    for parameter in response["Parameters"]:
        ...
        print(integration)
        if show_details:
            stages = [param.split("/")[-1] for param in response["Parameters"] if param.startswith(f"{prefix}{integration}/")]
...
def list_integration_details(integration, show_details):
    ...
    for parameter in response["Parameters"]:
        ...
        print(f"Stage: {stage}")
        if show_details:
            config = ssm.get_parameter(Name=parameter["Name"], WithDecryption=True)['Parameter']['Value']
...
if __name__ == "__main__":
    ...
    store_parser.add_argument('--stage', help='The stage of the integration', required=True)
    store_parser.add_argument('--integration', help='The integration name', required=True)
    store_parser.add_argument('--client-config', help='The client configuration JSON', required=True)
    store_parser.add_argument('--scopes', help='The scopes JSON array', required=True)

    list_parser = subparsers.add_parser('list')
    list_parser.add_argument('--details', action='store_true', help='Show details of each integration')

    details_parser = subparsers.add_parser('details')
    details_parser.add_argument('integration', help='The integration name')
    details_parser.add_argument('--details', action='store_true', help='Show details of each stage for the integration')
    ...
    if args.command == 'list':
        ...
        store_integration_config_as_secret(args.stage, args.integration, args.client_config, args.scopes)
