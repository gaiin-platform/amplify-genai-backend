...
from common.oauth_encryption import decrypt_oauth_data, encrypt_oauth_data, verify_oauth_encryption_parameter
...
def create_oauth_client(integration, client_config, scopes):
    ...
    match provider_case(integration):
        ...
        case IntegrationType.MICROSOFT:
            ...
            authority = f"{client_config.get('auth_uri')}{client_config.get('tenant_id')}"
...
def serialize_credentials(integration, credentials):
    ...
    match provider_case(integration):
        ...
        case IntegrationType.MICROSOFT:
            ...
            credentials_dict["expires_at"] = get_expiration_time(credentials_dict["expires_in"])
        case _:
            raise ValueError(f"Unsupported integration type: {integration}")    
...
def extract_refresh_response(integration, response_data, credentials):
    if "expires_in" not in response_data: # applied to both google and microsoft, if we add more this may need to move
        raise ValueError(f"Missing 'expires_in' in response data for integration {integration}")
...
def get_oauth_integration_parameter(integration):
    ...
    try:
        response = ssm.get_parameter(Name=f"/oauth/{parameter_name}", WithDecryption=True)
...
def start_auth(event, context, current_user, name, data):
    ...
    authorization_url, state = get_authorization_url_and_state(integration, auth_client, scopes)
...
def update_oauth_user_credentials(current_user, integration, credentials_data):
    ...
    try:
        ...
        return {"success": False, "message": f"Error retrieving existing OAuth credentials"}
...
def auth_callback(event, context):
    ...
    credentials = acquire_token_from_code(integration, client, scopes, authorization_code)
    ...
    update_res = update_oauth_user_credentials(current_user, integration, credentials_data)
...
def list_connected_integrations(event, context, current_user, name, data):
    ...
    if (not supported_integrations):
        return {"success": False, "message": f"Error retrieving user integrations from Admin Table"}

    connected = list_user_integrations(supported_integrations, current_user)
    print(False if connected is None else True)

    return {
        "success": False if connected is None else True,
        "data": connected
    }
...
def list_user_integrations(supported_integrations, current_user):
    ...
    for provider in integration_providers:
        ...
        try:
            ...
            record = response.get('Item')
            if record and 'integrations' in record:
                available_integrations = [integration['id'] for integration in supported_integrations[provider] if 'id' in integration]
                        
                integration_map = record['integrations']
                integration_ids = list(integration_map.keys())
                # only keep those that are still made available 
                filtered_ids = [id for id in integration_ids if id in available_integrations]
                connected_list.extend(filtered_ids)
        except Exception as e:
            print(f"Error retrieving record for integration {provider} for user {current_user}: {str(e)}")
            continue
    ...
    try:
        ...
        if record:
            ...
            if integration in integration_map:
                ...
                print(f"Successfully updated record for user {current_user} after deleting integration {integration}")
                return True
            else:
                print(f"Integration {integration} not found in record {item_key}")
        else:
            print(f"No record found in DynamoDB for user {current_user} and integration provider {integration_provider}")
    except Exception as e:
    ...
    try:
        ...
        if 'Item' in response:
            ...
            for provider, integrations in integrations_map.items():
                ...
                filtered_list = [integration for integration in integrations if integration.get("isAvailable", False)]
...
def regiser_secret(event, context, current_user, name, data):
    ...
    if not tenant_id: tenant_id = "amplifygenai" 

    param_name = build_integration_parameter_name(integration_provider)

    try:
        configuration = format_integration_param(integration_provider, client_id, client_secret, scopes[integration_provider], tenant_id)
        response = store_secret_parameter(param_name, json.dumps(configuration), "/oauth")
        if response:
...
def format_integration_param(integration_provider, client_id, client_secret, integration_scopes, tenant_id):
    OAUTH_AUDIENCE = os.environ.get('OAUTH_AUDIENCE')

    param_data = {"client_id": client_id, 
    ...
    match integration_provider:
        case IntegrationType.GOOGLE.value:
            ...
            param_data["auth_provider_x509_cert_url"] = "https://www.googleapis.com/oauth2/v1/certs"
        ...
        case IntegrationType.MICROSOFT.value:
            ...
            param_data["token_uri"] = f"https://login.microsoftonline.com/{tenant_id}/oauth2/v2.0/token"

    configuration = {
            "client_config": {"web": param_data},
...
def refresh_credentials(current_user, integration, credentials):
    ...
    if not refresh_token:
        ...
        return {"success": False, "message": "No refresh token available for refreshing credentials."}
    ...
    updated_credentials = extract_refresh_response(integration, response.json(), credentials)
