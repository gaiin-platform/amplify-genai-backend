...
def get_endpoint_data(parsed_data, model_name):
    ...
    endpoint_data = next((model[model_name] for model in parsed_data["models"] if model_name in model), None)
    ...
    deployment = deployment_parts[1].split("/")[0] if len(deployment_parts) > 1 else None
