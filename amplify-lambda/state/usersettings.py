...
def get_settings(event, context, current_user, name, data):
    try:
        ...
        response = users_table.scan(
            FilterExpression=Attr('user').eq(current_user)
        )
        ...
        if items:
            ...
            return {
                'success': True,
                'data': settings_item.get('settings', None)
            }
        else:
        ...
        print(f"An error occurred while retrieving settings for user {current_user}: {e}")
        return {
            'success': False,
    ...
    try:
       # Step 1: Query using the secondary index to get the primary key
        ...
        if not items:
            ...
            id_key = '{}/{}'.format(current_user, str(uuid.uuid4()))  # add the user's name to the key in DynamoDB
            new_item = {
                'id': id_key,
                'user': current_user,
                'name': '/state/share',
        ...
        return {
            'success': False,
            'error': 'Error occured while saving settings: {e}'
            }
