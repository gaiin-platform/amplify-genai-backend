    ...
    for message_id in message_ids:
        ...
        for field in fields:
            if field == 'id':
                ...
                result['attachments'] = [part for part in msg['payload'].get('parts', []) if 'filename' in part and part['filename']]
            elif field == 'sender':
                result['sender'] = next((header['value'] for header in headers if header['name'].lower() == 'from'), 'Unknown')
            elif field == 'subject':
                result['subject'] = next((header['value'] for header in headers if header['name'].lower() == 'subject'), 'No Subject')
            elif field == 'labels':
                ...
                result['cc'] = next((header['value'] for header in headers if header['name'].lower() == 'cc'), '')
            elif field == 'bcc':
                result['bcc'] = next((header['value'] for header in headers if header['name'].lower() == 'bcc'), '')
            elif field == 'deliveredTo':
                result['deliveredTo'] = next((header['value'] for header in headers if header['name'].lower() == 'delivered-to'), '')
            elif field == 'receivedTime':
                result['receivedTime'] = next((header['value'] for header in headers if header['name'].lower() == 'received'), '')
            elif field == 'sentTime':
                result['sentTime'] = next((header['value'] for header in headers if header['name'].lower() == 'date'), '')
            elif field in headers:
                result[field] = next((header['value'] for header in headers if header['name'].lower() == field.lower()), '')
