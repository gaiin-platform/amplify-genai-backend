...
def check_event_conflicts(current_user, proposed_start_time, proposed_end_time, 
                         return_conflicting_events=False, calendar_ids=None, 
                         check_all_calendars=False, access_token=None):
    """
...
def list_calendars(current_user, access_token=None):
    ...
    for calendar in calendar_list.get('items', []):
        ...
        calendars.append({
            ...
            'isShared': is_shared,
            'isOwned': is_owned,
            'owner': calendar.get('owner', {}).get('displayName', '') if is_shared else '',
            'ownerEmail': calendar.get('owner', {}).get('email', '') if is_shared else ''
        })
    
    return calendars

def create_calendar(current_user, name, description=None, timezone=None, access_token=None):
    """
...
def update_calendar_permissions(current_user, calendar_id, email, role='reader', 
                               send_notification=False, notification_message=None, 
                               access_token=None):
    """
...
def create_recurring_event(current_user, title, start_time, end_time, description=None, 
                          location=None, attendees=None, recurrence_pattern=None, 
                          calendar_id='primary', access_token=None):
    """
...
def add_event_reminders(current_user, event_id, reminders=None, calendar_id='primary', 
                       access_token=None):
    """
    ...
    for reminder in reminders:
        reminder_list.append(f"{reminder['method']} ({reminder['minutes']} minutes before)")
...
def update_calendar(current_user, calendar_id, name=None, description=None, timezone=None, access_token=None):
    """
