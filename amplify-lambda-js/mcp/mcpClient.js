...
export class MCPClient {
     ...
     * @param {Object} [attachmentContext] - Optional { images: [...], documents: [...] } from the conversation
     */
    async executeTool(toolName, args, attachmentContext) {
        if (!this.connected) {
        ...
        // Merge attachment context into args under the reserved _attachments key so
        // MCP servers that understand the convention can access images and documents.
        const enrichedArgs = (attachmentContext && Object.keys(attachmentContext).length > 0)
            ? { ...args, _attachments: attachmentContext }
            : args;

        if (attachmentContext && Object.keys(attachmentContext).length > 0) {
            logger.debug(`Forwarding attachments to MCP tool: images=${attachmentContext.images?.length || 0}, documents=${attachmentContext.documents?.length || 0}`);
        }
