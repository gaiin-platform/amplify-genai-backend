...
**Location**: `amplify-lambda-js/litellm-layer/build-python-litellm-layer.sh`
...
- Produces `litellm-layer/layer-build-arm64/python-litellm-arm64.zip` (~12-25 MB)
...
layers:
  pythonLiteLLM:
    path: litellm-layer/layer-build-arm64/layer
...
```bash
# Navigate to litellm-layer directory
cd amplify-lambda-js/litellm-layer

# Build the layer (includes prerequisite check)
./build-layer.sh
```
...
1. **"Python executable not found"**
   - Verify layer path in `serverless.yml`: `path: litellm-layer/layer-build-arm64/layer`
...
1. Update pins in `amplify-lambda-js/litellm-layer/build-python-litellm-layer.sh`
...
- **Build Guide**: `amplify-lambda-js/litellm-layer/README-PBS.md`
- **Quick Start**: `amplify-lambda-js/litellm-layer/QUICK_START_PBS.md`
- **This Summary**: `amplify-lambda-js/litellm-layer/IMPLEMENTATION_SUMMARY.md`
...
1. **Build the layer**: `cd amplify-lambda-js/litellm-layer && ./build-layer.sh`
