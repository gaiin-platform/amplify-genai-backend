...
docker run --rm --platform linux/amd64 \
  ...
  -c '
    ...
    # Copy ONLY essential stdlib modules for LiteLLM runtime
    ...
    # Copy lib-dynload (C extension modules - required)
    cp -r /var/lang/lib/python3.11/lib-dynload /output/lib/python3.11/ 2>/dev/null || true

    # Copy site-packages directory structure if it exists
    if [ -d "/var/lang/lib/python3.11/site-packages" ]; then
      mkdir -p /output/lib/python3.11/site-packages
    fi
...
AFTER_PHASE2=$(du -sk python | cut -f1)
PHASE2_SAVED=$(((AFTER_PHASE1 - AFTER_PHASE2) / 1024))
...
AFTER_PHASE3=$(du -sk python | cut -f1)
PHASE3_SAVED=$(((AFTER_PHASE2 - AFTER_PHASE3) / 1024))
...
AFTER_PHASE4=$(du -sk python | cut -f1)
PHASE4_SAVED=$(((AFTER_PHASE3 - AFTER_PHASE4) / 1024))
...
AFTER_PHASE5=$(du -sk python | cut -f1)
PHASE5_SAVED=$(((AFTER_PHASE4 - AFTER_PHASE5) / 1024))
...
AFTER_PHASE6=$(du -sk python | cut -f1)
PHASE6_SAVED=$(((AFTER_PHASE5 - AFTER_PHASE6) / 1024))
echo "  Saved: ${PHASE6_SAVED}MB" | tee -a $SAVINGS_LOG

# PHASE 7: Strip debug symbols from .so files using Docker (5-15MB) - BIG WIN
echo "Phase 7: Stripping debug symbols from .so files (using Docker)..."
SO_BEFORE=$(du -sk python | cut -f1)

# Use Docker with strip utility since .so files are Linux binaries
docker run --rm --platform linux/amd64 \
  -v $(pwd)/python:/python \
  --entrypoint /bin/bash \
  public.ecr.aws/lambda/python:3.11 \
  -c "find /python -name '*.so*' -type f -exec strip --strip-debug {} \; 2>/dev/null || true"

SO_AFTER=$(du -sk python | cut -f1)
PHASE7_SAVED=$(((SO_BEFORE - SO_AFTER) / 1024))
echo "  Saved: ${PHASE7_SAVED}MB ⭐ BIGGEST SAVER" | tee -a $SAVINGS_LOG
...
AFTER_PHASE8=$(du -sk python | cut -f1)
PHASE8_SAVED=$(((SO_AFTER - AFTER_PHASE8) / 1024))
echo "  Saved: ${PHASE8_SAVED}MB" | tee -a $SAVINGS_LOG

# PHASE 9: Remove unused packages that LiteLLM doesn't need at runtime
echo "Phase 9: Removing unused packages..."
# openai Python SDK (not needed - using LiteLLM abstraction)
rm -rf python/openai 2>/dev/null || true
# Hugging Face XET (large, rarely used)
rm -rf python/hf_xet 2>/dev/null || true
AFTER_PHASE9=$(du -sk python | cut -f1)
PHASE9_SAVED=$(((AFTER_PHASE8 - AFTER_PHASE9) / 1024))
echo "  Saved: ${PHASE9_SAVED}MB" | tee -a $SAVINGS_LOG

# PHASE 10: Find and report largest remaining packages
echo ""
echo "Phase 10: Analyzing largest remaining packages..."
echo "" >> $SAVINGS_LOG
echo "Largest Packages (Top 20):" >> $SAVINGS_LOG
du -sh python/* 2>/dev/null | sort -rh | head -20 | tee -a $SAVINGS_LOG

# Calculate final size and total savings
FINAL_SIZE=$(du -sk python | cut -f1)
TOTAL_SAVED=$(((INITIAL_SIZE - FINAL_SIZE) / 1024))
FINAL_SIZE_MB=$((FINAL_SIZE / 1024))

# Calculate percentage (avoid bc dependency)
if [ $INITIAL_SIZE -gt 0 ]; then
    PERCENT_SAVED=$(((TOTAL_SAVED * 100) / (INITIAL_SIZE / 1024)))
else
    PERCENT_SAVED=0
fi
...
echo "======================================"
echo ""
printf "%-25s %10s MB\n" "Initial size:" "$((INITIAL_SIZE / 1024))"
printf "%-25s %10s MB\n" "Final size:" "$FINAL_SIZE_MB"
printf "%-25s %10s MB (%s%%)\n" "Total saved:" "$TOTAL_SAVED" "$PERCENT_SAVED"
echo ""
...
if [ $FINAL_SIZE_MB -le 150 ]; then
    ...
    echo "  2. Remove unused LiteLLM providers"
fi
