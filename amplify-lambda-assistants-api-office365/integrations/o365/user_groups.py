...
def handle_graph_error(response: requests.Response) -> None:
    ...
    raise UserGroupError(f"Graph API error: {error_message} (Status: {response.status_code})")

def list_users(current_user: str, search_query: Optional[str] = None,
               top: int = 10, skip: int = 0,
               order_by: Optional[str] = None, access_token: str = None) -> List[Dict]:
    """
...
def list_groups(current_user: str, search_query: Optional[str] = None,
                group_type: Optional[str] = None, top: int = 10,
                skip: int = 0, access_token: str = None) -> List[Dict]:
    """
...
def get_group_details(current_user: str, group_id: str, access_token: str = None) -> Dict:
    """
...
def create_group(current_user: str, display_name: str, 
                mail_nickname: str, group_type: str = "Unified",
                description: Optional[str] = None,
                owners: Optional[List[str]] = None,
                members: Optional[List[str]] = None, access_token: str = None) -> Dict:
    """
    Creates a new group.
    ...
    try:
        ...
        if owners:
            body["owners@odata.bind"] = [f"{GRAPH_ENDPOINT}/users/{owner}" for owner in owners]
        if members:
            body["members@odata.bind"] = [f"{GRAPH_ENDPOINT}/users/{member}" for member in members]
