...
def handle_graph_error(response: requests.Response) -> None:
    ...
    raise OneNoteError(f"Graph API error: {error_message} (Status: {response.status_code})")

def list_notebooks(current_user: str, top: int = 10, access_token: str = None) -> List[Dict]:
    """
...
def list_sections_in_notebook(current_user: str, notebook_id: str, access_token: str) -> List[Dict]:
    """
...
def list_pages_in_section(current_user: str, section_id: str, access_token: str) -> List[Dict]:
    """
...
def create_page_in_section(current_user: str, section_id: str, 
                          title: str, html_content: str, access_token: str) -> Dict:
    """
...
<body>
    ...
    <object data-attachment="{file_name}" data="name:fileBlock1" type="{file_content_type}" />
        ...
        # Image part
        payload.extend(f"--{boundary}\r\n".encode('utf-8'))
        payload.extend(f'Content-Disposition: form-data; name="imageBlock1"\r\n'.encode('utf-8'))
        ...
        # File part
        payload.extend(f"--{boundary}\r\n".encode('utf-8'))
        payload.extend(f'Content-Disposition: form-data; name="fileBlock1"\r\n'.encode('utf-8'))
...
def format_notebook(notebook: Dict) -> Dict:
    ...
    return {
        ...
        'createdBy': notebook.get('createdBy', {}).get('user', {}).get('displayName', ''),
        'lastModifiedBy': notebook.get('lastModifiedBy', {}).get('user', {}).get('displayName', ''),
...
def format_section(section: Dict) -> Dict:
    ...
    return {
        ...
        'createdBy': section.get('createdBy', {}).get('user', {}).get('displayName', ''),
        'lastModifiedBy': section.get('lastModifiedBy', {}).get('user', {}).get('displayName', ''),
...
def format_page(page: Dict) -> Dict:
    ...
    return {
        ...
        'lastModifiedBy': page.get('lastModifiedBy', {}).get('user', {}).get('displayName', ''),
