...
load()
schema_description = describe_database_schema()
...
@chain
def parse_sql_code(string):
    pattern = r"```sql[\s\S]*?```"

    # Find the first occurrence of the SQL code block
    match = re.search(pattern, string, re.IGNORECASE | re.DOTALL)

    if match:
        # Extract the contents of the code block
        sql_code = match.group(0)[7:-3].strip()
        return sql_code
    else:
        return None
...
schema_prompt = ("system", schema_description)
