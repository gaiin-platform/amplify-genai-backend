...
from llm.chat import prompt, chat, chat_simple
...
def get_description_and_schema(access_token, model, code):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_to_task = {
            executor.submit(chat_simple, access_token, model, f"Please describe what the following code does and provide a sample of the data it returns:\n\n{code}", "Carefully describe what this code does in 1-2 sentences and include a sample of the data it returns."): 'description',

            executor.submit(chat_simple, access_token, model, f"Please describe the schema for the context used by this code:\n\n{code}", "Carefully create a json schema for context object based on what the code accesses within it. If the context value is open-ended, then just create an object with \"additionalProperties\": true and no keys defined. Output the schema in EXACTLY ONE ```json block."): 'schema',

            executor.submit(chat_simple, access_token, model, f"Please describe the top-level parameters:\n\n{code}", """
            ...
            executor.submit(chat_simple, access_token, model, f"Please describe the schema for the return value(s) of this script:\n\n{code}", "Carefully create a json schema for the return value from the code. If the return value is open-ended, then just create an object with \"additionalProperties\": true and no keys defined. Output the schema in EXACTLY ONE ```json block."): 'return_schema'
        }
    ...
    return_schema_dict = json.loads(return_schema.split("```json")[1].split("```")[0].strip())
...
@vop(
    ...
    params={
        "requested_function": "Description of the requested function that the Python script should perform.",
        "libraries": "A list of available Python libraries and their versions to use (e.g. [{'name': 'numpy', 'version': '1.21.2'}]).",
        "notes": "Optional notes to provide additional context for the code generation."
    }
)
@validated(op="create")
def create_code(event, context, current_user, name, data):
    try:
        ...
        raw_code = chat_simple(access_token, model, prompt_instructions, system_instructions)
        ...
        description, schema_dict, agent_desc, return_schema_dict = get_description_and_schema(access_token, model, code)
...
@vop(
    ...
    params={
        "s3_key": "The S3 key where the Python script is stored.",
        "context": "The context to provide to the Python script for execution."
    }
)
@validated(op="invoke")
def invoke_code(event, context, current_user, name, data):
    try:
        ...
        if not uuid_str or not context_dict:
            ...
            return {
                'success': False,
                'message': "uuid and context are required."
            }
        ...
        if not item:
            ...
            return {
                'success': False,
                'message': f"No entry found for uuid: {uuid_str}"
            }

        # Confirm the current_user is the creator
        creator = item.get('creator', {}).get('S')
        if creator != current_user:
            print(f"Error: current_user {current_user} is not the creator of uuid: {uuid_str}")
            return {
                'success': False,
    ...
    try:
        ...
        for item in items:
            functions.append({
                'uuid': item['uuid']['S'],
                's3Key': item['s3Key']['S'],
                'description': item.get('description', {}).get('S', 'No description available'),
        ...
        return {
            'success': False,
            'message': f"Failed to list user functions: {str(e)}"
        }
