...
class Map(Step):
    def __init__(self, id: str, data: Dict):
        ...
        if 'map' not in data or 'input' not in data:
            raise WorkflowValidationError(f"Map step {id} must have 'map' and 'input' fields")
...
class Reduce(Step):
    def __init__(self, id: str, data: Dict):
        ...
        if 'input' not in data:
            raise WorkflowValidationError(f"Reduce step {id} must have 'input' and 'output' fields")
...
class Route(Step):
    def __init__(self, id: str, data: Dict):
        ...
        if 'input' not in data or 'route' not in data:
            raise WorkflowValidationError(f"Route step {id} must have 'input' and 'route' fields")
...
class If(Step):
    def __init__(self, id: str, data: Dict):
        ...
        if 'condition' not in data or 'then' not in data:
            raise WorkflowValidationError(f"If step {id} must have 'condition' and 'then' fields")
...
def validate_workflow(workflow: Workflow, path: str = ""):
    if not isinstance(workflow, Workflow):
        raise WorkflowValidationError(f"{path}: Expected Workflow, got {type(workflow)}")

    for i, step in enumerate(workflow.steps):
        step_path = f"{path}.steps[{i}]" if path else f"steps[{i}]"

        if not isinstance(step, (Step, Workflow, Map, Reduce, Route, If, Action)):
            raise WorkflowValidationError(f"{step_path}: Invalid step type {type(step)}")
