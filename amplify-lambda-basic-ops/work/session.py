...
def add_record(username, session_id, record_data, attachments=None):
    ...
    # Update session
    update_session(username, session_id, {
...
def delete_record(username, session_id, record_id):
    try:
        # Check if session is active
        session = sessions_table.get_item(Key={'username': username, 'session_id': int(session_id)})['Item']
        if not session or session['status'] != 'active':
            raise ValueError("Session is not active")

        # Get the record before deleting it
        record = records_table.get_item(Key={'session_id': int(session_id), 'record_id': int(record_id)})
        if 'Item' not in record:
            raise ValueError("Record not found")
        record = record['Item']

        # Delete record
        records_table.delete_item(Key={'session_id': int(session_id), 'record_id': int(record_id)})
        ...
        # Update session
        update_session(username, session_id, {
            'records_count': max(0, session['records_count'] - 1),  # Ensure count doesn't go negative
...
def list_records(username, session_id):
    try:
        # Check if session is active
        session = sessions_table.get_item(Key={'username': username, 'session_id': int(session_id)})
...
def fetch_record(username, session_id, record_id):
    try:
        # Check if session is active
        session_response = sessions_table.get_item(Key={'username': username, 'session_id': int(session_id)})
        if 'Item' not in session_response or session_response['Item']['status'] != 'active':
            raise ValueError("Session is not active or does not exist")

        # Fetch the record
        record_response = records_table.get_item(Key={'session_id': int(session_id), 'record_id': int(record_id)})
        if 'Item' not in record_response:
            raise ValueError(f"Record with id {record_id} not found in session {session_id}")
...
def fetch_attachment(username, session_id, record_id, attachment_name):
    try:
        ...
        if not s3_key:
            raise ValueError(f"Attachment {attachment_name} not found for record {record_id}")
